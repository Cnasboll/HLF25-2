...
There are plenty of unit tests. `HLF25-2\v03\tests\json_mapping_test.dart` shows how the entire example json blob is parsed to a `HeroModel`. The editing done by the CLI is in fact using json as an intermediate format so that the app is ready to be connected to the API without further modifications. `HLF25-2\v03\tests\sql_generation_test.dart` shows expected SQL that is generated, but the reason I don't type it directly but  generate it from metadata in the `Field<T,V>`-definitions is simply to be able to prevent bugs when changing something in the structure. Code generation *always* saves time in the end.

Also note that the parser will throw if given conflicting height or weight information, see  `HLF25-2\v03\tests\weight_test.dart` and `HLF25-2\v03\tests\height_test.dart`, respectively and in particular the consistency checking in `HLF25-2\v03\value_types\value_type.dart` :
